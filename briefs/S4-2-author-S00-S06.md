# Ticket 4.2 — Author scenes S00–S06 (content only)

## Goal
Author the Phase-1 curriculum scenes S00, S04, S05, S06 and upgrade the existing S01–S03 to the stage-4 content shape, in `content/phase1/scenes.json` (and `content/phase1/world.json` slot pools if a scene needs a new pool). Design authority, read in this order: `content/phase1/scene-list.md` (rows S00–S06: ID, NPC, kind, requires, new words per exchange), `docs/design/GDD.md` §5.2 Dialogue and comprehension, §5.3 Content and hidden repetition, §5.6 Consequences, §3 Cast (voices, personalities), §8 First three minutes (S00); `docs/design/TDD.md` §1.1 (new field semantics), §6.1 audio ID naming; `docs/PLAN.md` content contract; `content/README.md`. Study the three existing scenes in `scenes.json` as the style reference.
...
- S00 (arrival, landlord, story) introduces the starter ten in five exchanges exactly as scene-list.md says, and the first three minutes must match GDD §8.
- S04/S06 (mentor, tea house) e4 is a review exchange: no new words, tests 2–3 of the scene's words.

## Scope out
Scenes S07+, audio files, engine, types, checker, world layout.

## Acceptance
- `scenes.json` parses; 7 scenes with curriculumIndex 0–6; each has 4–6 exchanges; every rule above holds by your own reading (re-scan every Chinese string against words.json before hand-back).
