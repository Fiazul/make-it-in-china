...
describe('economy and day cycle', () => {
  ...
  it.each(['story', 'consequence'] as const)('%s does not consume a slot', kind => {
  ...
  it.each([0, 1, 2, 100])('caps consequence cost %s to a single 1–5 yuan penalty', cost => {
    const data = withConsequence(); data.scenes[1].cost = cost; const game = createGame(data); game.start('job'); game.reply(1);
    expect(game.state().wallet).toBe(20 - Math.max(1, Math.min(5, cost))); game.reply(0);
    expect(game.state().wallet).toBe(20 - Math.max(1, Math.min(5, cost)));
  });
  ...
  it('keeps insufficient rent in the wallet and grants renewable grace without debt', () => {
    ...
    for (let i = 0; i < 4; i++) game.sleep(); expect(game.state()).toMatchObject({ day: 14, wallet: 5, rentDue: true });
  });
