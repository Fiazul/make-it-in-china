...
import { CommandError, type GameState } from './types';
...
export function nextDay(state: GameState) {
  state.day++; state.actionSlots = state.rules.actionSlots; spend(state, state.rules.foodCost);
  if (state.day % 7 === 0 && !state.rentDue) { state.rentDue = true; state.graceUntil = state.day + state.rules.graceDays; }
  settleRent(state);
  // An expired grace period is extended, never converted into debt or eviction.
